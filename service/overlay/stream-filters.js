...
(function (root, factory) {
    ...
    var BD_RE = /(^|[^a-z0-9])(bd(-?(rips?|mv|remux|r|iso|box))?|remux|jpbd|usbd|brrip)(?=[^a-z]|$)|blu-?ray/i;
    ...
    var WEB_RE = /(^|[^a-z0-9])(web-?dl|web-?rip|web|cr|crunchyroll|hidive|amzn|nf|netflix|b-?global|bilibili|bili|funimation|adn|dsnp|hulu)(?=[^a-z]|$)/i;
    ...
    var AUDIO_OK = /(^|[^a-z0-9])(ddpa?|dd\+|dd|e-?ac-?3|eac3|ac-?3|dolby[ ._-]?digital([ ._-]?plus)?|he-?aac|aac|flac|opus|vorbis|mp3|l?pcm)(?=[^a-z]|$)/i;
    var AUDIO_NO = /(^|[^a-z0-9])(dts(-?(hd|x|es|ma|hdma|hd[ ._-]?ma))?|dts[ ._:-]?x|true-?hd)(?=[^a-z]|$)/i;
    ...
    function audioSupport(t) {
        ...
        // "Atmos" with no carrier named: Dolby Digital Plus on web releases (fine),
        // TrueHD on disc/remux releases (silent).
        if (!ok && ATMOS_RE.test(t) && !TRUEHD_RE.test(t)) { if (isBD(t)) return 'unsupported'; ok = true; }
