...
test("coordinator imports and queues a source, then ignores ordinary heartbeats", async () => {
  const calls = [];
  const jellyfin = {
    syncViewers: async (id, state, options) => calls.push(["sync", id, state.positionSeconds, options]),
  };
  const mediaLibrary = {
  ...
  assert.deepEqual(calls, [
    ...
    ["sync", "jellyfin-item", 10, { actions: ["queue"], checkDrift: true }],
    ["sync", "jellyfin-item", 11, { actions: [], checkDrift: false }],
  ]);
});
...
test("coordinator serializes updates and forwards only meaningful transitions", async () => {
  const calls = [];
  const jellyfin = {
    syncViewers: async (id, state, options) => calls.push({ id, position: state.positionSeconds, ...options }),
  };
  ...
  assert.deepEqual(calls.at(-1), {
    id: "item", position: 80, actions: ["pause", "seek"], checkDrift: false,
  });
});
...
test("slow initial indexing cannot turn queued heartbeats into false seeks", async () => {
  ...
  assert.deepEqual(calls[1].actions, []);
});

test("duplicate and stale state samples are ignored", async () => {
  const calls = [];
  const jellyfin = {
    syncViewers: async (...args) => calls.push(args),
  };
